...
在 Thread 类中有相应的无参构造和有参构造，会调用 init() 方法完成线程属性的初始化。源码如下：
...
private int            priority; // 线程优先级 1~10级
private boolean     single_step; // 线程是否单步执行 默认 false
private boolean     daemon = false; // 线程是否为守护线程 (见目录：守护线程)
...
private long stackSize;  //此线程请求的堆栈大小，如果创建者未指定堆栈大小，则为 0。最终由虚拟指定；一些虚拟机会忽略它。
private long nativeParkEventPointer; //在本机线程终止后持续存在的 JVM 私有状态 (ToDo: 不知道有什么作用)
private long tid; 线程 ID：线程的唯一标识
...
![Thread状态转换图](../图片/3.thread-status.png)
...
安全管理器 是一个定义安全策略的对象。此策略指定不安全或敏感的操作。安全策略不允许的任何操作都会抛出 SecurityException 异常。 应用程序还可以查
询其安全管理器允许哪些操作。

具体使用详见：https://www.cnblogs.com/yiwangzhibujian/p/6207212.html